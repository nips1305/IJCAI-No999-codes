...
def cal_clustering_inner_metric(x, pre_label):
    if np.unique(pre_label).shape[0] !=1:
        sc = metrics.silhouette_score(x, pre_label)
        dbi = metrics.davies_bouldin_score(x, pre_label)
        ch = metrics.calinski_harabasz_score(x, pre_label)
        print(f"sc {sc:.4f}, dbi {dbi:.4f}, ari {ch:.4f}")
